...
if user_name == "admin" and password == "admin":
    class CarParking():
        '''
        class for creating parking and its method
        '''
        ...
        def park_car(self, car_no, car_colour):
            '''
            method to collect the data for car parking
            '''
        ...
        def leave_car(self, slot_no):
            '''
            method works at the time when car is leaving from parking lot
            '''
        ...
        def get_reg_no_by_colour(self, color_name):
            '''
            method to get the registration no by color
            '''
            all_car = []
        ...
        def get_slot_no_by_colour(self, color_name):
            ''' method to get the slot no by color'''
            all_slot_no = []
        ...
        def get_slot_no_by_reg(self, reg_no):
            ''' method to get the slot no by registration no.
            '''
            all_slot_no = []
            for key in self.parking_data:
                i = self.parking_data[key]
        ...
        def status(self):
            '''
            method to get the status of parking lot
            '''
    ...
    # works when you chosse option for file
    if choice == 1:
        parking_lot = None
        with open('command.txt', 'r') as commands:
            for command in commands:
                ...
                elif "create_parking_lot" in command and len(command) == 1:
                    parking_lot = CarParking()
    ...
    # works when you choose option for command line
    elif choice == 2:
